...
async fn main() -> Result<()> {
    ...
    let ciphertext1 = seal
        .encrypt::<TheAlgorithm>(&key, KEY_ID.to_string())
        .to_vec(plaintext)?;
    let pending_decryptor1 = seal.decrypt().slice(&ciphertext1)?;
    let key_id = pending_decryptor1.key_id().unwrap();
    let key_bytes = key_store.get_key(key_id).unwrap();
    let decrypted1 = pending_decryptor1.with_key_bytes(key_bytes)?;
    ...
    let key_bytes = key_store.get_key(key_id).unwrap();
    let decrypted2 = pending_decryptor2.with_key_bytes(key_bytes)?;
    ...
    let key_bytes = key_store.get_key(key_id).unwrap();
    let mut decryptor3 = pending_decryptor3.with_key_bytes(key_bytes)?;
    ...
    let key_bytes = key_store.get_key(key_id).unwrap();
    let mut decryptor4 = pending_decryptor4.with_key_bytes(key_bytes)?;
    ...
    seal.encrypt::<TheAlgorithm>(&key, KEY_ID.to_string())
    ...
    let key_bytes = key_store.get_key(key_id).unwrap();
    pending_decryptor5.with_key_bytes_to_writer(key_bytes, &mut decrypted5)?;
    ...
    let key_bytes = key_store.get_key(key_id).unwrap();
    let decrypted6 = pending_decryptor6.with_aad(aad).with_key_bytes(key_bytes)?;
    ...
    let key_bytes = key_store.get_key(key_id).unwrap();
    let result_fail2 = pending_fail2.with_key_bytes(key_bytes);
