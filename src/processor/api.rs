...
impl<'a> EncryptionConfigurator<'a> {
    ...
    pub fn into_parallel_streaming_flow<W: Write + Send + 'a>(
        ...
        mut writer: W,
        channel_bound: usize,
    ) -> Result<ParallelEncryptionStreamFlow<'a, W>> {
        self.header.write_to_prefixed_writer(&mut writer)?;
    ...
    pub async fn into_async_flow<W: AsyncWrite + Send + Unpin + 'a>(
        ...
        mut writer: W,
        channel_bound: usize,
    ) -> Result<AsyncEncryptionStreamFlow<'a, W>> {
        self.header
...
pub struct EncryptionFlow<'a, W: Write + 'a> {
    writer: W,
    config: EncryptionConfigurator<'a>,
}

impl<'a, W: Write + 'a> EncryptionFlow<'a, W> {
...
impl<'a> EncryptionFlow<'a, Vec<u8>> {
...
pub struct ParallelEncryptionStreamFlow<'a, W: Write + Send + 'a> {
    writer: W,
    config: EncryptionConfigurator<'a>,
    channel_bound: usize,
}

impl<'a, W: Write + Send + 'a> ParallelEncryptionStreamFlow<'a, W> {
...
#[cfg(feature = "async")]
pub struct AsyncEncryptionStreamFlow<'a, W: AsyncWrite + Send + Unpin + 'a> {
    writer: W,
    config: EncryptionConfigurator<'a>,
    channel_bound: usize,
}
...
#[cfg(feature = "async")]
impl<'a, W: AsyncWrite + Send + Unpin + 'a> AsyncEncryptionStreamFlow<'a, W> {
...
pub fn prepare_decryption_from_slice(
    ciphertext: &[u8],
) -> Result<(PendingDecryption<&[u8]>, &[u8])> {
    let (header, body) = read_header_from_slice(ciphertext)?;
...
pub fn prepare_decryption_from_reader<R: Read>(
    mut reader: R,
) -> Result<PendingDecryption<impl Read>> {
...
pub struct PendingDecryption<S> {
    header: SealFlowEnvelopeHeader,
    source: S,
}

impl<S> PendingDecryption<S> {
    /// Returns a reference to the parsed header.
    pub fn header(&self) -> &SealFlowEnvelopeHeader {
        &self.header
    }
}

impl<'a> PendingDecryption<&'a [u8]> {
...
impl<'a, R: Read + 'a> PendingDecryption<R> {
...
impl<'a, R: Read + Send + 'a> PendingDecryption<R> {
...
#[cfg(feature = "async")]
impl<'a, R: AsyncRead + Send + Unpin + 'a> PendingDecryption<R> {
