...
use crate::processor::traits::{FinishingWrite, SealFlowHeaderExt};
...
impl<'a, H: SealFlowHeader> EncryptionConfigurator<'a, H> {
    ...
    pub fn into_writer<W: Write + 'a>(
        ...
        self.header.ext_write_to_prefixed_writer(&mut writer)?;
        Ok(EncryptionFlow {
            writer,
            config: self,
        })
    ...
    pub async fn into_async_flow<W: AsyncWrite + Send + Unpin + 'a>(
        ...
        self.header
            .ext_write_to_prefixed_async_writer(&mut writer)
            .await?;
...
impl<'a, W: Write + 'a, H: SealFlowHeader> EncryptionFlow<'a, W, H> {
    pub fn encrypt_ordinary(self, plaintext: &[u8]) -> Result<Vec<u8>> {
        let symmetric_params = self.config.header.ext_symmetric_params().clone();
        ...
        let mut header_bytes = self.config.header.ext_encode_to_prefixed_vec()?;
...
pub fn prepare_decryption_from_slice<H: SealFlowHeader>(
    ...
    let (header, body) = H::ext_decode_from_prefixed_slice(ciphertext)?;
...
pub fn prepare_decryption_from_reader<R: Read, H: SealFlowHeader>(
    ...
    let header = H::ext_decode_from_prefixed_reader(&mut reader)?;
...
impl<'a, H: SealFlowHeader> PendingDecryption<&'a [u8], H> {
    ...
    pub fn decrypt_ordinary(
        ...
        let params = self.header.ext_symmetric_params();
...
pub fn read_header_from_slice<H: SealFlowHeader>(ciphertext: &[u8]) -> Result<(H, &[u8])> {
    H::ext_decode_from_prefixed_slice(ciphertext)
}
...
pub fn read_header_from_reader<R: Read, H: SealFlowHeader>(reader: &mut R) -> Result<H> {
    H::ext_decode_from_prefixed_reader(reader)
}
...
pub async fn read_header_from_async_reader<R: AsyncRead + Unpin + Send, H: SealFlowHeader>(
    ...
    H::ext_decode_from_prefixed_async_reader(reader).await
}