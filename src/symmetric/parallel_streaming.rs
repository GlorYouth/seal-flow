        ...
        s.spawn(move || {
            raw_chunk_rx
                ...
                .for_each(|(index, chunk)| {
                    ...
                    let encrypted = S::encrypt(key, &nonce, &chunk, None).map_err(Error::from);
    ...
    thread::scope(|s| {
        ...
        s.spawn(move || {
            enc_chunk_rx
                ...
                .for_each(|(index, chunk)| {
                    let nonce = derive_nonce(&base_nonce, index);
                    let decrypted = S::decrypt(key, &nonce, &chunk, None).map_err(Error::from);
...
mod tests {
    ...
    fn test_parallel_streaming_roundtrip() {
        ...
        encrypt::<AesGcmScheme<Aes256>, _, _>(
            &key,
            &mut source,
            &mut encrypted_dest,
        ...
        decrypt::<AesGcmScheme<Aes256>, _, _>(&key, &mut encrypted_source, &mut decrypted_dest)
            .unwrap();
    ...
    fn test_wrong_key_fails() {
        ...
        let mut decrypted_dest = Vec::new();
        let result =
            decrypt::<AesGcmScheme<Aes256>, _, _>(&key2, &mut encrypted_source, &mut decrypted_dest);
        assert!(result.is_err());
    }
} 