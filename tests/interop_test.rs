...
use seal_flow::processor::api::{
    prepare_decryption_from_reader, prepare_decryption_from_slice, EncryptionConfigurator,
};
#[cfg(feature = "async")]
...
async fn encrypt_asynchronous(
    ...
    let configurator = EncryptionConfigurator::new(header, Cow::Borrowed(key), aad);
    let mut ciphertext = Vec::new();
    let flow = configurator
        .into_async_flow(&mut ciphertext, 4)
        .await?;
...
async fn test_all_modes_interoperability() -> anyhow::Result<()> {
    ...
    enum EncFn {
        Sync(Arc<dyn Fn(&TypedSymmetricKey, Option<Vec<u8>>, &[u8]) -> anyhow::Result<Vec<u8>> + Send + Sync>),
        Async(Arc<dyn for<'a> Fn(&'a TypedSymmetricKey, Option<Vec<u8>>, &'a [u8]) -> Pin<Box<dyn Future<Output = anyhow::Result<Vec<u8>>> + Send + 'a>> + Send + Sync>),
    }
    ...
    let encryption_modes = {
        let mut modes = vec![
            ...
            ("ParallelStreaming", EncFn::Sync(Arc::new(encrypt_parallel_streaming))),
        ];
        #[cfg(feature = "async")]
        modes.push((
            "Asynchronous",
    ...
    type DecryptFn = Arc<dyn for<'a> Fn(&'a [u8], &'a TypedSymmetricKey, Option<Vec<u8>>) -> DecryptResult<'a> + Send + Sync>;

    let decryption_modes: Vec<(&str, DecryptFn)> = {
        let mut modes: Vec<(&str, DecryptFn)> = vec![
            ("Ordinary", Arc::new(|ciphertext, key, aad| Box::pin(async move {
                let pending = prepare_decryption_from_slice::<TestHeader>(ciphertext)?;
                pending.decrypt_ordinary(Cow::Borrowed(key), aad).map_err(|e| e.into())
            }))),
            ("Parallel", Arc::new(|ciphertext, key, aad| Box::pin(async move {
                let pending = prepare_decryption_from_slice::<TestHeader>(ciphertext)?;
                pending.decrypt_parallel(Cow::Borrowed(key), aad).map_err(|e| e.into())
            }))),
            ("Streaming", Arc::new(|ciphertext, key, aad| Box::pin(async move {
            ...
            ("Parallel Streaming", Arc::new(|ciphertext, key, aad| Box::pin(async move {
                let mut reader = Cursor::new(ciphertext);
                let pending = prepare_decryption_from_reader::<_, TestHeader>(&mut reader)?;
                let mut writer = Vec::new();
                pending.decrypt_parallel_streaming(&mut writer, Cow::Borrowed(key), aad, 4)?;
                Ok(writer)
            }))),
        ];

        #[cfg(feature = "async")]
        modes.push(("Asynchronous", Arc::new(|ciphertext, key, aad| Box::pin(async move {
            use tokio::io::AsyncReadExt;
            let mut reader = tokio::io::BufReader::new(ciphertext);
            let pending = prepare_decryption_from_async_reader::<_, TestHeader>(&mut reader).await?;
